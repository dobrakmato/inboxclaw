...
@click.option("--no-confirm", is_flag=True, default=False, help="Do not mark events as processed after extraction.")
def pull(config_path: Optional[str], sink_name: Optional[str], event_type: Optional[str], batch_size: Optional[int], no_confirm: bool):
    ...
    try:
        with httpx.Client(timeout=10.0) as client:
            ...
            # 2. Mark Processed (unless --no-confirm)
            if no_confirm:
                return
            ...
            confirm_response = client.post(mark_url, params={"batch_id": batch_id})
            
            if confirm_response.status_code != 200:
                click.secho(f"Failed to mark batch {batch_id} as processed: {confirm_response.text}", fg="red", err=True)
                sys.exit(1)
