...
class TestRestartCommand(unittest.TestCase):
    ...
    def test_restart_user_success(self, mock_load_config, mock_run, mock_getuid):
    ...
    @patch('src.cli.commands.restart.subprocess.run')
    @patch('src.cli.commands.restart.load_config')
    def test_restart_system_success(self, mock_load_config, mock_run, mock_getuid):
        # Test system restart
    ...
    @patch('src.cli.commands.restart.get_uid', return_value=1000)
    @patch('src.cli.commands.restart.load_config')
    def test_restart_system_no_root(self, mock_load_config, mock_getuid):
    ...
    @patch('src.cli.commands.restart.subprocess.run')
    @patch('src.cli.commands.restart.load_config')
    def test_restart_custom_name(self, mock_load_config, mock_run, mock_getuid):
